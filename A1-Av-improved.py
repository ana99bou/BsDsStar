...
def jackmass(t1,i):
    ...
    return (((sum_with_exceptions_jack(av1n0, t1, i)))/(np.sqrt(1/3*(jack(avdx[t1],i)+jack(avdy[t1],i)+jack(avdz[t1],i))*jack(avb[dt-t1],i))))*np.sqrt((4*md*mb)/(np.exp(-md*(t1))*np.exp(-mb*(dt-(t1)))))*pre
