...
mom=[[''],
     ['final_state_GX/operator_GammaXGamma5/n2_1/1_0_0','final_state_GY/operator_GammaYGamma5/n2_1/0_1_0', 'final_state_GZ/operator_GammaZGamma5/n2_1/0_0_1'],
     ...
     ['final_state_GX/operator_GammaXGamma5/n2_4/2_0_0','final_state_GY/operator_GammaYGamma5/n2_4/0_2_0', 'final_state_GZ/operator_GammaZGamma5/n2_4/0_0_2'],
...
for j in range(dt):
    ...
    for k in range(nconf):
        ...
        av1n0ya[j, k] = tmp1ya
        av1n0za[j, k] = tmp1za
        av1n0xma[j, k] = tmp1xma
        av1n0yma[j, k] = tmp1yma
        av1n0zma[j, k] = tmp1zma
...
for j in range(dt):
    ...
    for i in range(nconf):
        x=x+(pre*(pre2*(jack(av1n0xa[j],i)-jack(av1n0ya[j],i)+jack(av1n0za[j],i)+jack(av1n0xma[j],i)-jack(av1n0yma[j],i)+jack(av1n0zma[j],i))/nmom2*(1/md-1)*(mb+md0)-(((jack(av1n0x[j],i)-jack(av1n0y[j],i)+jack(av1n0z[j],i)+jack(av1n0xm[j],i)-jack(av1n0ym[j],i)+jack(av1n0zm[j],i)+jack(av1n0xn[j],i)-jack(av1n0yn[j],i)+jack(av1n0zn[j],i)+jack(av1n0xmn[j],i)-jack(av1n0ymn[j],i)+jack(av1n0zmn[j],i))/nmom)/(1/3*np.sqrt((jack(avdx[j],i)+jack(avdy[j],i)+jack(avdz[j],i))*jack(avb[j],i)))*np.sqrt((4*md*mb)/(np.exp(-md*j)*np.exp(-mb*(dt-j))))))-avn0[j])**2
...
def jackmass(t1,i):
    ...
    return pre*((pre2*(jack(av1n0xa[t1],i)-jack(av1n0ya[t1],i)+jack(av1n0za[t1],i)+jack(av1n0xma[t1],i)-jack(av1n0yma[t1],i)+jack(av1n0zma[t1],i))/nmom2*(1/md-1)*(mb+md0))-(((jack(av1n0x[t1],i)-jack(av1n0y[t1],i)+jack(av1n0z[t1],i)+jack(av1n0xm[t1],i)-jack(av1n0ym[t1],i)+jack(av1n0zm[t1],i)+jack(av1n0xn[t1],i)-jack(av1n0yn[t1],i)+jack(av1n0zn[t1],i)+jack(av1n0xmn[t1],i)-jack(av1n0ymn[t1],i)+jack(av1n0zmn[t1],i))/nmom)/(1/3*np.sqrt((jack(avdx[t1],i)+jack(avdy[t1],i)+jack(avdz[t1],i))*jack(avb[t1],i))))*np.sqrt((4*md*mb)/(np.exp(-md*(t1))*np.exp(-mb*(dt-(t1))))))
