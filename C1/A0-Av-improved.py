...
for j in range(dt):
    ...
    for k in range(nconf):
        ...
        tmpb = (np.real(bsn0[k, j]) + np.real(bsn0[k, ts-1-j])) / 2 if j != 0 else np.real(bsn0[k, 0])
...
def jackmass(t1,i):
    ...
    return (((sum_with_exceptions_jack(av1n0, t1, i)))/(np.sqrt(1/3*(jack(avdx[t1],i)+jack(avdy[t1],i)+jack(avdz[t1],i))*jack(avb[t1],i))))*np.sqrt((4*dsfit['EffectiveMass'][i]*bsfit['EffectiveMass'][i])/(np.exp(-dsfit['EffectiveMass'][i]*(t1))*np.exp(-bsfit['EffectiveMass'][i]*(dt-(t1)))))*pre
