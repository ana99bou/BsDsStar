...
for j in range(dt+1):    
    avn0[j] = pre * (sum_with_exceptions(fin3pt[j], nsq) / ( np.sqrt(1/3*(findx[j] + findy[j] + findz[j]) * finb[dt-j]))) * np.sqrt((4 * mb * md) / (np.exp(-md * j) * np.exp(-mb * (dt - j))))
...
for j in range(dt+1):
    ...
    for i in range(nconf):
        ...
        x=x+((sum_with_exceptions_jack(av1n0, nsq,j, i)/(np.sqrt(1/3*(jack(avdx[j],i)+jack(avdy[j],i)+jack(avdz[j],i))*jack(avb[dt-j],i))))*np.sqrt((4*dsfit['EffectiveMass'][i]*bsfit['EffectiveMass'][i])/(np.exp(-dsfit['EffectiveMass'][i]*j)*np.exp(-bsfit['EffectiveMass'][i]*(dt-j))))*pre-avn0[j])**2
...
def jackmass(t1,i):
    ...
    return (((sum_with_exceptions_jack(av1n0,nsq, t1, i)))/(np.sqrt(1/3*(jack(avdx[t1],i)+jack(avdy[t1],i)+jack(avdz[t1],i))*jack(avb[dt-t1],i))))*np.sqrt((4*md*mb)/(np.exp(-md*(t1))*np.exp(-mb*(dt-(t1)))))*pre
